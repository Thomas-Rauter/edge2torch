...
def test_compile_graph_recurrent_allows_cycles():
    edgelist = pd.DataFrame(
        {
            "source": ["node_a", "node_b"],
            "target": ["node_b", "node_a"],
...
def test_compile_graph_recurrent_returns_expected_feature_names():
    edgelist = pd.DataFrame(
        {
    ...
    _, artifact = compile_graph(edgelist, backend="recurrent")
...
def test_recurrent_model_raises_for_wrong_input_width():
    edgelist = pd.DataFrame(
        {
            "source": ["gene_1", "gene_2"],
            "target": ["output_1", "output_1"],

...
class Edge2TorchError(Exception):
    """Base exception for all KPNN-specific errors."""


class InputValidationError(Edge2TorchError):
    """Raised when public API inputs are invalid."""


class GraphValidationError(Edge2TorchError):
    """Raised when the graph is biologically or structurally invalid."""


class CompilationError(Edge2TorchError):
    """Raised when graph compilation fails."""


class InterpretationError(Edge2TorchError):

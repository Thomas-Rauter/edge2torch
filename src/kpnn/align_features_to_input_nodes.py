...
def align_features_to_input_nodes(
    ...
    raise KPNNError("Unsupported input data type.")
...
def _validate_align_features_to_input_nodes_inputs(
    ...
    """
    if not isinstance(artifact, KPNNArtifact):
        raise KPNNError("'artifact' must be a KPNNArtifact.")

    feature_names = getattr(artifact, "feature_names", None)

    if not isinstance(feature_names, list):
        raise KPNNError("'artifact.feature_names' must be a list of strings.")
    ...
    if ad is not None and isinstance(data, ad.AnnData):
        ...
        if len(set(var_names)) != len(var_names):
            raise KPNNError("AnnData var_names must not contain duplicates.")

...
| **Not supported** | Cyclic graphs (use `state_update`). Directed graphs that cannot be layerized into a feedforward execution order. Attention or message-passing layers as compiled primitives. |
...
| Category | |
|----------|---|
| **Well supported** | Cyclic graphs and feedback loops among named nodes. Static exogenous inputs on input nodes. Configurable unrolling via `steps`. Node interpretation at `step_*` sites, with optional summary aggregation across steps (`site_aggregation`). Signed or fixed edge weights. Post-core head and activation via `customize_model()`. Access compiled weights via `model.state_linear`. |
...
| **Not supported** | Sequence models with temporal input (LSTM, GRU, or an input tensor with a time dimension). Different learnable weight matrices per unrolled step. Gated recurrence inside the compiled graph. Adaptive iterate-until-convergence loops inside `compile_graph()`. Standard GNN layer types as compiled primitives. |
...
| Has cycles or feedback and static inputs per sample | `state_update` |
